...
- `rose check-llm` — Check LLM configuration and connectivity. Shows provider, model, and credential status. For ALCF provider, reads `ALCF_ACCESS_TOKEN` as the API key. For OpenAI/local, shows masked API key. Options: `--no-test`, `--json`. Reads `LLM_PROVIDER`, `LLM_MODEL`, `LLM_API_KEY` / `OPENAI_API_KEY`, `LLM_BASE_URL`, `LLM_TEMPERATURE`, `ALCF_ACCESS_TOKEN`, `ALCF_CLUSTER` from environment.
...
- **ALCF token handling**: When `LLM_PROVIDER=alcf`, the `ALCF_ACCESS_TOKEN` environment variable is used as the API key. No complex authentication workflow—just set the token in `.env` or environment. ALCF cluster endpoints: sophia = `https://inference-api.alcf.anl.gov/resource_server/sophia/vllm/v1`, metis = `https://inference-api.alcf.anl.gov/resource_server/metis/api/v1`. The `_get_llm_config()` function resolves `ALCF_ACCESS_TOKEN` into the `api_key` field when provider is ALCF.
