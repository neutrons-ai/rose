...
- **OpenAI-compatible LLM support**: Any OpenAI-compatible endpoint (OpenAI, ALCF, Ollama, vLLM, etc.) is supported via `LLM_BASE_URL` + `LLM_API_KEY`. No provider-specific code — just set the base URL and key in `.env`. `generate_model_yaml()` accepts `base_url` and `max_tokens` parameters.
...
- `rose check-llm` — Check LLM configuration and connectivity. Shows provider, model, credential status, and base URL. Options: `--no-test`, `--json`. Reads `LLM_PROVIDER`, `LLM_MODEL`, `LLM_API_KEY` / `OPENAI_API_KEY`, `LLM_BASE_URL`, `LLM_TEMPERATURE` from environment.
...
- **Generic LLM configuration**: All LLM endpoints (OpenAI, ALCF, Ollama, vLLM, etc.) use the same env vars: `LLM_API_KEY` for the key/token and `LLM_BASE_URL` for custom endpoints. No provider-specific env vars or code paths. For ALCF, just set `LLM_BASE_URL` to the cluster endpoint (e.g. `https://inference-api.alcf.anl.gov/resource_server/sophia/vllm/v1`) and `LLM_API_KEY` to the Globus access token.
